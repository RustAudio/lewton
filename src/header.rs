...
impl ResidueBook {
    ...
    fn read_book(rdr :&mut BitpackCursor,
        ...
        Ok(ResidueBook { vals_used : vals_used, val_i: val_i })
    }
}
...
fn read_mapping(rdr :&mut BitpackCursor,
    ...
    let mapping_submaps = if try!(rdr.read_bit_flag()) {
        try!(rdr.read_u4()) + 1 } else { 1 };
    let mapping_coupling_steps = if try!(rdr.read_bit_flag()) {
        try!(rdr.read_u8()) as u16 + 1 } else { 0 };
