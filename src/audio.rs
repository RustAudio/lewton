...
pub fn floor_one_decode(rdr :&mut BitpackCursor, codebooks :&[Codebook],
    ...
    for class in &fl.floor1_partition_class {
        ...
        let mut cval = if cbits > 0 {
            ...
            try!(rdr.read_huffman(&codebooks[cbook].codebook_huffman_tree))
        } else { 0 };
        for _ in 0 .. cdim {
...
fn extr_neighbor<F>(v :&[u32], x :usize,
    ...
    let (mut extr_idx, mut max_val) = sm.iter().cloned().enumerate()
            .find(|tu| compare(tu.1, bound)).expect(&format!(
                    "No index y < {} found where v[y] is {} than v[{}] = 0x{:08x}!",
                    x, s, x, bound));
...
fn low_neighbor(v :&[u32], x :usize) -> (usize, u32) {
    extr_neighbor(v, x, |a, b| {a < b}, "smaller")
}


fn high_neighbor(v :&[u32], x :usize) -> (usize, u32) {
    extr_neighbor(v, x, |a, b| {a > b}, "bigger")
}
...
pub fn floor_one_curve_compute_amplitude(floor1_y :&[u32], fl :&FloorTypeOne) -> (Vec<u32>, Vec<bool>) {
    let v = vec![256, 128, 86, 64];
    ...
    (floor1_final_y, floor1_step2_flag)
}
...
pub fn floor_decode<'a>(rdr :&mut BitpackCursor,
        ...
        floors :&'a [Floor]) -> Vec<(Option<Vec<u32>>, &'a Floor)> {
...
fn residue_packet_decode_inner(rdr :&mut BitpackCursor, cur_blocksize :u16,
        do_not_decode_flag :&[bool], resid :&Residue, codebooks :&[Codebook]) -> Result<Vec<f64>, ()> {
...
pub fn residue_packet_decode(rdr :&mut BitpackCursor, cur_blocksize :u16,
    ...
    if resid.residue_type == 2 {
        ...
        if !to_decode_found {
            ...
            Ok(vectors_deinterleaved)
        }
    } else {
        residue_packet_decode_inner(rdr, cur_blocksize,
...
pub fn read_audio_packet(ident :&IdentHeader, setup :&SetupHeader, packet :&[u8], pwr :&mut PreviousWindowRight)
    ...
    for (&mag, &angle) in
            mapping.mapping_magnitudes.iter().zip(mapping.mapping_angles.iter()) {
        let (mut mag_vector, mut angle_vector) = dual_mut_idx(&mut residue_vectors,
    ...
    // Inverse MDCT
    for ref mut spectrum in &mut audio_spectri {
